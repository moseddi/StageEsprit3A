...
import { PlusCircleOutlined, EditOutlined, DeleteOutlined, UserOutlined, MailOutlined, TeamOutlined, FilePdfOutlined } from '@ant-design/icons'; // Importez les icônes nécessaires, y compris FilePdfOutlined
import { fetchEtudiants, createEtudiant, updateEtudiant, deleteEtudiant, fetchClasses } from '../services/apiService';

const { Option } = Select;
const { Text } = Typography;

// Chargement des scripts jsPDF et jspdf-autotable via CDN
// En production, vous installeriez ces packages via npm (npm install jspdf jspdf-autotable)
// et les importeriez directement dans votre fichier JS.
const loadPdfLibs = () => {
...
const StudentManagement = ({ currentUser }) => {
    ...
    const [pdfLibsLoaded, setPdfLibsLoaded] = useState(false); // Nouvel état pour suivre le chargement des libs PDF

    // Charger les bibliothèques PDF au montage du composant
    useEffect(() => {
        loadPdfLibs()
    ...
    const loadEtudiants = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const data = await fetchEtudiants(currentUser.authToken);
            setEtudiants(data);
        } catch (error) {
    ...
    const loadClassesForDropdown = useCallback(async () => {
        ...
        try {
            ...
            message.error('Échec du chargement des classes pour la sélection: ' + error.message);
        }
    ...
    }, [loadEtudiants, loadClassesForDropdown]);
    ...
    const handleEtudiantSubmit = async () => {
        try {
            ...
            // Assurez-vous que classeId est un nombre si votre backend l'attend comme tel
    ...
    // NOUVEAU : Fonction pour exporter la liste des étudiants au format PDF
    ...
    const handleExportStudentsPdf = async () => {
        ...
        try {
            ...
            doc.autoTable({
                head: [tableColumn],
                body: tableRows,
                startY: 35, // Décalage pour laisser place au logo et titre
                theme: 'striped',
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    <EditOutlined className="text-red-500 cursor-pointer text-lg" onClick={() => {
                        setSelectedStudent(record);
                        form.setFieldsValue({ ...record, classeId: record.classe?.id });
                        setModalVisible(true);
                    }} />
                    <Popconfirm title="Êtes-vous sûr de supprimer cet étudiant?" onConfirm={() => handleDeleteEtudiant(record.id)} okText="Oui" cancelText="Non">
    ...
    if (currentUser?.role !== 'ADMIN') {
        return <Alert message="Accès non autorisé" description="Vous n'avez pas les permissions nécessaires pour voir cette section." type="error" showIcon className="rounded-lg shadow-md" />;
    }
    ...
    return (
        <div className="p-4">
            <Space className="mb-6">
                ...
                {/* NOUVEAU BOUTON : Exporter Étudiants en PDF */}
                <Button
                    type="default"
                    icon={<FilePdfOutlined />}
                    onClick={handleExportStudentsPdf}
                    loading={loading || !pdfLibsLoaded} // Désactivé si les libs ne sont pas chargées
            ...
            <Table
                ...
                // style={{ background: '#fff0f5', borderRadius: '12px', padding: '10px' }} // Déplacé vers className si possible ou laissé ici
            />
            <Modal
