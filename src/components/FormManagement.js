...
const FormManagement = ({ currentUser }) => {
    ...
    const [expirationDate, setExpirationDate] = useState(null); // <-- gestion date expiration
    ...
    // Mise à jour ici pour envoyer la date d'expiration (ISO string) ou null
    ...
    return (
        <div>
            ...
            <Modal
                title={<span style={{ color: '#ff69b4', fontSize: '20px', fontWeight: 'bold' }}>Générer lien d'évaluation</span>}
                ...
                footer={[
                    <Button
                        ...
                        style={{ backgroundColor: '#ff69b4', color: '#fff', borderRadius: '8px' }}
                    >
                        Copier le lien
                    </Button>,
                    <Button
                ...
                >
                    {questionsForSubmission.map((q) => (
                        <Form.Item
                            key={q.id}
                            name={`question_${q.id}`}
                            label={<Text strong style={{ color: '#c8102e' }}>{q.libelle}</Text>}
                    ...
                    <Form.Item>
                        <Button
                            ...
                            htmlType="submit"
                            size="large"
                            style={{ backgroundColor: '#ff69b4', borderRadius: '8px', fontWeight: 'bold' }}
                        >
                            Soumettre
                        </Button>
                    </Form.Item>
            ...
            <Modal
                title={<span style={{ color: '#ff69b4', fontSize: '20px', fontWeight: 'bold' }}>{formDetails ? `Détails du formulaire "${formDetails.titre}"` : 'Détails du formulaire'}</span>}
            ...
            >
                {formDetails ? (
                    <>
                        <p><strong>Titre:</strong> {formDetails.titre}</p>
                        <p><strong>Niveau:</strong> {formDetails.niveau}</p>
                        <p><strong>Description:</strong> {formDetails.description}</p>
                        <p><strong>Date de création:</strong> {new Date(formDetails.creationDate).toLocaleString()}</p>
                        <p><strong>Créateur:</strong> {formDetails.creator}</p>
                        <p><strong>Statut:</strong> {formDetails.statut ? 'Actif' : 'Inactif'}</p>
                        <h3 style={{ color: '#c8102e' }}>Questions</h3>
                        <ul>
